...
int main(int argc, char *argv[]) {
    if(argc == 1 || argc == 2) {
        puts("Pass all argument supplied");
        exit(1);
    }
    ...
    while(read(fd, &c, 1)) {
        if(charcheck(c) || numcheck(c) || c == '_') {
            ...
            if(i) {
                if(s_search(stop_words, word) == NULL)
                    para_words = p_insert(para_words, word, line);
            }
            memset(word, '\0', i+1);
            i = 0;
        }
    ...
    //printf("\nFinal Time - %ld %d", final.tv_sec, final.tv_usec);
